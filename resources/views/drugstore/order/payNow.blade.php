...
<div class="app-container">
    <div class="product-wrap">
        ...
        <form action="{{route('payNow.send')}}" class="payment-main" method="POST">
            @csrf
            @method('POST')
            <div class="payment-body__wrap">
                    ...
                    <div class="form-group" style="padding-bottom: 0">
                        ...
                        <div id="card-info" style="display: none;">
                            ...
                            <div class="card-info__item">
                                ...
                                <input type="text" id="cvv" name="cvv" value="{{session()->get('back.cvv')}}">
                                @else
                ...
                <div class="order-detail">
                    ...
                    <div class="order-detail__wrap">
                        ...
                        @if (session()->has('back.name') != null)
                        <ul class="order-list">
                            <li class="order-group">
                                <div>
                                    <span class="order-name">{{session()->get('back.name')}}</span>
                                    <input type="hidden" name="name" value="{{session()->get('back.name')}}">
                                    <span class="order-quantity">x{{session()->get('back.quantity')}}</span>
                                    <input type="hidden" name="quantity" value="{{session()->get('back.quantity')}}">
                                </div>
                                <div class="order-price__one">
                                    <span>{{session()->get('back.price')}}</span>
                                    <span>円</span>
                                    <input type="hidden" name="price" value="{{session()->get('back.price')}}">
                                    <input type="hidden" name="id" value="{{session()->get('back.id')}}">
                                </div>
                            </li>
                        </ul>
                        <div class="order-price">
                            <div class="order-price__all">
                                <div>ご注文合計（税抜）</div>
                                <div>{{session()->get('back.price')*session()->get('back.quantity')}}円</div>
                            </div>
                            <div class="order-price__tax">
                                <div>消費税</div>
                                <div>{{session()->get('back.price')*session()->get('back.quantity')*0.1}}円</div>
                            </div>
                            <div class="order-price__final">
                                <div>ご注文合計（税込）</div>
                                <div>{{session()->get('back.price')*session()->get('back.quantity')*1.1}}円</div>
                            </div>
                        </div>
                        @else
                        <ul class="order-list">
